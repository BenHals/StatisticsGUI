...
function oneMeanCanvas(inputData, heading, statistic){
	this.xScale = null;
...
this.draw = function(){
	...
	var sampleMeans = [];
	var container = d3.select("#rightHalf");
	this.canvas = container.append("canvas");
	this.context = this.canvas.node().getContext("2d");
	this.custom = container.append("custom");
	this.custom.append("custom").attr("id", "population");
	this.custom.append("custom").attr("id", "sampleLines");
	this.custom.append("custom").attr("id", "meanCircles");
	...
	var circle = this.custom.selectAll("custom.circle").data(this.population);
	   circle.enter().append("custom.circle")
	...
	this.custom.append("custom.line").attr("x1", this.xScale(this.populationStatistic)).attr("y1", this.windowHelper.section1.bottom+20).attr("x2", this.xScale(this.populationStatistic)).attr("y2", this.windowHelper.section1.bottom-20).style("stroke-width", 2).style("stroke", "black");
	...
	d3.timer(this.drawCanvas(this));
}
this.drawCanvas = function(runningObj){
	var self = runningObj;
	var elements = self.custom.selectAll("custom.circle").each(function(d){
		var node = d3.select(this);
		self.context.beginPath();
		self.context.rect(node.attr("cx"), node.attr("cy"),10,10);
		self.context.fill();
